(ns doi-system.main
  "Kotodama WASM entrypoint for the doi_system clean-room actor (L4) — Clojure port.
  ...
  facts (`doi_system.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "doi_system")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"       :plural "works"       :id-prefix "doisyste_wor"
    :fields [:doi :title :type :publishedAt]   :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"     :plural "authors"     :id-prefix "doisyste_aut"
    :fields [:orcid :givenName :familyName]    :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"   :plural "citations"   :id-prefix "doisyste_cit"
    :fields [:workId :citedDoi]                :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"   :plural "standards"   :id-prefix "doisyste_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"     :plural "records"     :id-prefix "doisyste_rec"
    :fields [:accession :database :title]      :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"    :plural "subjects"    :id-prefix "doisyste_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"doi_system.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "doi_system-compat" :tier tier :entities entities} 200])
...
(defn start! [] :doi_system-compat/ready)
